(() => {
  ...
  const ensureBackgroundAssetPath = (asset) => {
    if (!asset) {
      return '';
    }
    const trimmed = trimLeadingRelative(asset);
    if (!trimmed) {
      return '';
    }
    if (trimmed.startsWith('assets/')) {
    ...
    if (trimmed.startsWith('images/backgrounds/')) {
      return `assets/${trimmed}`;
    }
    if (trimmed.includes('/')) {
      return `assets/${trimmed}`;
    }
    return `assets/images/backgrounds/${trimmed}`;
  };
  const resolveBackgroundAsset = (asset) => resolveAssetPath(ensureBackgroundAssetPath(asset));
  const resolvePuzzleImage = (image) => (isExternalUrl(image) ? image : resolveBackgroundAsset(image || 'forest.jpg'));
...
const ADVENTURE_PRESETS = {
  ...
  const initProfileSetup = () => {
    ...
    if (avatars.length > 0) {
      const storedAvatar = getUserProfile()?.avatar || '';
      const normalisedStoredAvatar = storedAvatar ? ensureAvatarAssetPath(storedAvatar) : null;

      avatars.forEach((avatar) => {
        const avatarPath =
          avatar.dataset.avatar || avatar.querySelector('img')?.getAttribute('src') || '';
        const normalizedAvatarPath = ensureAvatarAssetPath(avatarPath);

        if (normalisedStoredAvatar && normalisedStoredAvatar === normalizedAvatarPath) {
        ...
        avatar.addEventListener('click', () => {
          ...
          if (avatarPath) {
            updateUserProfile({ avatar: normalizedAvatarPath });
          }
        });
      });
    }
  ...
  const bindProfileResetTriggers = () => {
    document.querySelectorAll('[data-change-profile]').forEach((trigger) => {
      ...
      trigger.addEventListener('click', (event) => {
        ...
        const target = trigger.dataset.profileTarget || 'pages/choose.html';
  ...
  const initAdventureSelection = () => {
    ...
    adventureCards.forEach((card) => {
      ...
      const puzzleData = {
        ...
        image: resolvePuzzleImage(card.dataset.image || fallback.image)
      };
  ...
  const initAnimatedBackground = () => {
    ...
    const backgroundThemes = {
      turtle: { gradient: 'linear-gradient(180deg, #a5d6a7, #e8f5e9)', asset: resolveBackgroundAsset('leaf.png') },
      drop: { gradient: 'linear-gradient(180deg, #81d4fa, #b3e5fc)', asset: resolveBackgroundAsset('bubbles.png') },
      squirrel: { gradient: 'linear-gradient(180deg, #ffe082, #fff8e1)', asset: resolveBackgroundAsset('leaf.png') },
      sun: { gradient: 'linear-gradient(180deg, #fff59d, #f1f8e9)', asset: resolveBackgroundAsset('clouds.png') },
      default: { gradient: 'linear-gradient(180deg, #b3e5fc, #e0f7fa)', asset: resolveBackgroundAsset('clouds.png') }
    };
  ...
  document.addEventListener('ecolud:user-ready', () => {
    ...
    const loadStoredPuzzle = () => {
      ...
      return {
        ...
        image: resolvePuzzleImage(stored.image || fallback.image)
      };
    };
