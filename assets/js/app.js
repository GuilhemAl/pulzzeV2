(() => {
  ...
  const normalizeAssetPath = (assetPath) => {
    ...
    const trimmed = trimLeadingRelative(assetPath);
    return trimmed.startsWith("assets/") ? trimmed : `assets/${trimmed}`;
  };
  const ensureAvatarAssetPath = (avatarPath) => {
    if (!avatarPath || isExternalUrl(avatarPath)) {
      return avatarPath || "";
    }
    const trimmed = trimLeadingRelative(avatarPath);
    if (trimmed.startsWith("assets/images/avatars/")) {
      return trimmed;
    }
  ...
  const resolveRootRelative = (relativePath) => {
    ...
    return `${ROOT_PREFIX}${relativePath}`;
  };
  const resolveAssetPath = (assetPath) => {
    if (!assetPath || isExternalUrl(assetPath)) {
      return assetPath || "";
    }
    return resolveRootRelative(normalizeAssetPath(assetPath));
  };
  ...
  const translations = {
    fr: {
      ...
      "game-reward-tree": "Un puzzle achet\u00E9 = 🌳 Un arbre plant\u00E9 avec Reforest'Action",
    ...
    en: {
      ...
      "game-reward-tree": "One puzzle purchased = 🌳 One tree planted with Reforest'Action",
    ...
    es: {
      ...
      "game-reward-tree": "Un rompecabezas comprado = 🌳 Un \u00E1rbol plantado con Reforest'Action",
  ...
  const deserializeUser = (rawValue) => {
    ...
    try {
      ...
      const normalized = {
        ...
        avatar: ensureAvatarAssetPath(typeof parsed.avatar === "string" ? parsed.avatar : ""),
  ...
  const refreshNavSession = () => {
    ...
    if (navAvatar) {
      const avatarPath = ensureAvatarAssetPath(user?.avatar || "");
      if (avatarPath) {
        navAvatar.src = resolveAssetPath(avatarPath);
      } else {
  ...
  const storeUser = (user) => {
    ...
    const normalizedAvatar = ensureAvatarAssetPath(
  ...
  const navigateTo = (url) => {
    if (typeof url === "string" && url.trim()) {
      window.location.href = resolveRootRelative(url.trim());
    }
  };
  ...
  const initNavToggle = () => {
    ...
    if (!toggle) {
      ...
      const navId = nav.id || 'primaryNav';
      nav.id = navId;
      toggle.setAttribute('aria-controls', navId);
      const icon = document.createElement('img');
      icon.src = `${ROOT_PREFIX}assets/images/icons/6499731.png`;
      icon.alt = '';
    ...
    const iconEl = toggle.querySelector('img');
    if (iconEl && !iconEl.getAttribute('aria-hidden')) {
    ...
    const updateLabel = (expanded) => {
      const labels = labelMap[currentLang] || labelMap.fr;
  ...
  const initPlayTriggers = () => {
    ...
    const handlePlayClick = (event) => {
      ...
      if (hasCompleteProfile(user)) {
        navigateTo('pages/game.html');
      } else {
        navigateTo('pages/choose.html');
      }
    };
  ...
  const PUZZLE_PAGE_PATTERN = /\/pages\/(puzzle(?:3d|-[^\/]+)\.html)$/;
  let puzzleResetExecuted = false;

  const isPuzzlePage = () => {
    try {
      return PUZZLE_PAGE_PATTERN.test(window.location.pathname);
    } catch (error) {
      console.warn('[PuzzleReset] Failed to inspect location pathname.', error);
      return false;
    }
  };

  const decodeIframeSrc = (value) =>
    typeof value === 'string' ? value.replace(/&amp;/g, '&') : '';

  const generateCacheBustedSrc = (src) => {
    if (!src) {
      return src;
    }
    try {
      ...
      console.warn('[PuzzleReset] Failed to build URL from iframe src.', error);
      const separator = src.includes('?') ? '&' : '?';
      return `${src}${separator}r=${Date.now()}`;
    }
  };

  const maybeOpenForceFresh = (baseSrc) => {
    if (!baseSrc) {
      return false;
    }
    try {
      const params = new URLSearchParams(window.location.search);
      if (params.get('forceFresh') === '1') {
        window.open(generateCacheBustedSrc(baseSrc), '_blank', 'noopener');
        puzzleResetExecuted = true;
        return true;
      }
    } catch (error) {
      console.warn('[PuzzleReset] Failed while processing forceFresh flag.', error);
    }
    return false;
  };

  const resetPuzzleFrame = () => {
    if (puzzleResetExecuted || !isPuzzlePage()) {
      return;
    }
    ...
    const baseSrc = decodeIframeSrc(
      frame.getAttribute('data-puzzle-src') || frame.getAttribute('src')
    );
    if (!baseSrc) {
      return;
    }
    if (maybeOpenForceFresh(baseSrc)) {
      return;
    }
    frame.setAttribute('src', generateCacheBustedSrc(baseSrc));
    puzzleResetExecuted = true;
  };

  const bootPuzzleFrame = () => {
    puzzleResetExecuted = false;
    resetPuzzleFrame();
  };
